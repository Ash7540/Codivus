...
class SecretsVisitor(ast.NodeVisitor):
    ...
    def _check_secret_value(
        self, var_name: str, val: str, lineno: int, is_secret_var: bool
    ):
        ...
        if is_pattern_match or (
            is_secret_var and shannon_entropy(val) > 3.0 and len(val) >= 8
        ):
            ...
            masked_val = (
                val[:4] + "*" * (len(val) - 6) + val[-2:] if len(val) > 6 else "****"
            )
