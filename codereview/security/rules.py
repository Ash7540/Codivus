    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute):
            if isinstance(node.func.value, ast.Name):
                ...
                elif node.func.value.id == 'algorithms' and node.func.attr in ('DES', 'DES3', 'ARC4', 'Blowfish'):
                    is_weak_cipher = True
            ...
            if fullname:
                if '.modes.ECB' in fullname:
                    is_weak_cipher = True
                    cipher_detail = "AES ECB mode"
                elif '.algorithms.DES' in fullname or '.algorithms.DES3' in fullname or '.algorithms.ARC4' in fullname or '.algorithms.Blowfish' in fullname:
                    is_weak_cipher = True
    ...
    def visit_ImportFrom(self, node):
        if node.module:
            for name_node in node.names:
                if name_node.name in ('DES', 'DES3', 'ARC4', 'Blowfish') and 'algorithms' in node.module:
