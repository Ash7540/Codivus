...
class Reviewer:
    ...
    def review_file(
        ...
        cached_result = self.cache.get(
            abs_filepath, code_content, modified_lines, category_focus
        )
        if cached_result:
        ...
        with time_operation("Static Analysis"):
            try:
                ...
                raise StaticAnalysisError(
                    f"Static analysis failed for {abs_filepath}: {str(e)}"
                )
        ...
        if category_focus:
            static_issues = [
                i for i in static_issues if i.category.lower() == category_focus.lower()
            ]

        # 3. Invoke LLM provider with CodeContext and static analysis findings
        def prompt_modifier_fn(p):
            return self.plugin_manager.modify_prompt(code_context, p)
        ...
        with time_operation("LLM Generation"):
            try:
                ...
                raise LLMProviderError(
                    f"LLM provider failed for {abs_filepath}: {str(e)}"
                )
        ...
        if category_focus:
            combined_issues = [
                ...
                if i.category.lower() == category_focus.lower()
            ]
        ...
        critical_count = sum(
            1 for i in combined_issues if i.severity.lower() == "critical"
        )
        ...
        if static_issues:
            summary_text += (
                f" Static analysis discovered {len(static_issues)} additional issues."
            )
        ...
        # Cache result
        self.cache.set(
            abs_filepath, code_content, result, modified_lines, category_focus
        )

        return result

    def review_dir(
        self, dirpath: str, category_focus: Optional[str] = None
    ) -> RepositoryReviewResult:
        """
        ...
        for full_path, rel_path in all_py_files:
            ...
            for imp in context.imports:
                resolved = self._resolve_import_to_local_file(
                    imp.name, abs_dirpath, full_path
                )
            ...
            broken_imports = self._check_broken_local_imports(
                context.imports, abs_dirpath, rel_path, all_rel_files
            )
            all_static_issues.extend(broken_imports)

            file_summaries.append(
                f"- File `{rel_path}`: {review_res.summary.summary_text}"
            )
        ...
        critical_count = sum(
            1 for i in all_combined_issues if i.severity.lower() == "critical"
        )
        high_count = sum(1 for i in all_combined_issues if i.severity.lower() == "high")
        medium_count = sum(
            1 for i in all_combined_issues if i.severity.lower() == "medium"
        )
        ...
        avg_overall = sum(r.score.overall_score for r in file_reviews.values()) / max(
            1, len(file_reviews)
        )
        avg_security = sum(r.score.security_score for r in file_reviews.values()) / max(
            1, len(file_reviews)
        )
        avg_performance = sum(
            r.score.performance_score for r in file_reviews.values()
        ) / max(1, len(file_reviews))
        avg_style = sum(r.score.style_score for r in file_reviews.values()) / max(
            1, len(file_reviews)
        )
    ...
    def review_staged(
        self, repo_path: str = ".", category_focus: Optional[str] = None
    ) -> RepositoryReviewResult:
        """
        ...
        dep_graph = {}
        all_rel_files = {
            os.path.relpath(f, repo.repo_path).replace("\\", "/") for f in staged_files
        }
        ...
        for full_path in staged_files:
            ...
            review_res = self.review_file(
                full_path, modified_lines, category_focus=category_focus
            )
            file_reviews[rel_path] = review_res
            ...
            for imp in context.imports:
                resolved = self._resolve_import_to_local_file(
                    imp.name, repo.repo_path, full_path
                )
                if resolved and resolved in all_rel_files:
                    deps.append(resolved)
            dep_graph[rel_path] = deps
            ...
            broken_imports = self._check_broken_local_imports(
                context.imports, repo.repo_path, rel_path, all_rel_files
            )
            broken_imports = [
                i
                for i in broken_imports
                if i.line_number is None or i.line_number in modified_lines
            ]
            all_static_issues.extend(broken_imports)

            file_summaries.append(
                f"- File `{rel_path}` (staged): {review_res.summary.summary_text}"
            )
    ...
    def review_commit(
        ...
        dep_graph = {}
        all_rel_files = {
            os.path.relpath(f, repo.repo_path).replace("\\", "/") for f in commit_files
        }
        ...
        for full_path in commit_files:
            ...
            file_summaries.append(
                f"- File `{rel_path}` (commit {commit_hash}): {review_res.summary.summary_text}"
            )
    ...
    def review_diff(
        self, ref_range: str, repo_path: str = ".", category_focus: Optional[str] = None
    ) -> RepositoryReviewResult:
        """
        ...
        dep_graph = {}
        all_rel_files = {
            os.path.relpath(f, repo.repo_path).replace("\\", "/") for f in diff_files
        }
        ...
        for full_path in diff_files:
            ...
            file_summaries.append(
                f"- File `{rel_path}` (diff {ref_range}): {review_res.summary.summary_text}"
            )
    ...
    def _generate_folder_tree(self, dirpath: str, excludes: Set[str]) -> str:
        ...
        def walk_tree(current_dir, prefix=""):
            ...
            entries = [
                e for e in entries if e not in excludes and not e.startswith(".")
            ]
    ...
    def _resolve_import_to_local_file(
        self, import_name: str, root_dir: str, current_file: str
    ) -> Optional[str]:
    ...
    def _check_broken_local_imports(
        self, imports_list, root_dir: str, file_rel_path: str, all_rel_files: Set[str]
    ) -> List[Issue]:
        issues = []
        for imp in imports_list:
            ...
            if os.path.exists(local_candidate_dir) or os.path.exists(
                local_candidate_file
            ):
                resolved_rel = self._resolve_import_to_local_file(
                    import_name, root_dir, os.path.join(root_dir, file_rel_path)
                )
