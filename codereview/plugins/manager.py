...
class PluginManager:
    ...
    def load_plugins(self) -> None:
        ...
        try:
            ...
            for ep in eps:
                try:
                    ...
                    print(
                        f"Warning: Failed to load entry point plugin {ep.name}: {str(e)}",
                        file=sys.stderr,
                    )
        except Exception:
            pass
        ...
        if os.path.exists(self.local_plugins_dir) and os.path.isdir(
            ...
            for filename in os.listdir(self.local_plugins_dir):
                if filename.endswith(".py") and not filename.startswith("_"):
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Failed to load local plugin {filename}: {str(e)}",
                            file=sys.stderr,
                        )
    ...
    def get_analysers(self) -> List[Callable[[CodeContext], List[Issue]]]:
        ...
        for plugin in self.plugins:
            try:
                ...
                print(
                    f"Warning: Plugin {plugin.name} get_analysers failed: {str(e)}",
                    file=sys.stderr,
                )
        return analysers
    ...
    def modify_prompt(self, context: CodeContext, prompt: str) -> str:
        ...
        for plugin in self.plugins:
            try:
                ...
                print(
                    f"Warning: Plugin {plugin.name} modify_prompt failed: {str(e)}",
                    file=sys.stderr,
                )
        return prompt
    ...
    def on_review_start(self, context: CodeContext) -> None:
        ...
        for plugin in self.plugins:
            try:
                ...
                print(
                    f"Warning: Plugin {plugin.name} on_review_start failed: {str(e)}",
                    file=sys.stderr,
                )
    ...
    def on_review_end(self, context: CodeContext, result: ReviewResult) -> None:
        ...
        for plugin in self.plugins:
            try:
                ...
                print(
                    f"Warning: Plugin {plugin.name} on_review_end failed: {str(e)}",
                    file=sys.stderr,
                )
