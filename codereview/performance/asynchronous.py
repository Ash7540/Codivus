...
class AsyncPerformanceVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        if self.in_async_func:
            ...
            elif isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and node.func.value.id in ('requests', 'urllib') and node.func.attr in ('get', 'post', 'put', 'delete', 'request', 'urlopen'):
                is_blocking = True
                func_detail = f"blocking sync call '{node.func.value.id}.{node.func.attr}()'"
