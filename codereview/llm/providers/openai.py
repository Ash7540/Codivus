...
"""
        ...
        try:
            completion = self.client.beta.chat.completions.parse(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a senior software architect. Analyze the repository structure and summary of findings to provide a professional overview."},
                    {"role": "user", "content": user_prompt},
                ],
            ...
            if parsed is None:
                raise ValueError("Failed to parse structured repository summary from OpenAI.")
