...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if first_arg not in subcommands and first_arg not in {"-h", "--help", "-v", "--version"}:
    ...
    # 3. security parser
    security_parser = subparsers.add_parser("security", help="Scan codebase specifically for security vulnerabilities")
    ...
    # 4. diff parser
    diff_parser = subparsers.add_parser("diff", help="Review differences between git references")
    ...
    # 5. config parser
    config_parser = subparsers.add_parser("config", help="View or modify local configuration")
    ...
    config_parser.add_argument(
        "key",
        nargs="?",
        help="Config key name (required for set)"
    )
    config_parser.add_argument(
        "value",
        nargs="?",
        help="Config key value (required for set)"
    )

    # 6. report parser
    report_parser = subparsers.add_parser("report", help="Convert an existing JSON report to other formats")
    report_parser.add_argument(
        "json_file",
        help="Path to the existing JSON review report"
    )
    report_parser.add_argument(
        "--format",
        choices=["text", "json", "markdown", "html", "sarif", "pdf"],
    ...
    report_parser.add_argument(
        "--output",
        type=str,
        help="Save converted report to file path"
    )
