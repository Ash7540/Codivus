...
def handle_config(args, config):
    ...
    if subcommand == "init":
        if os.path.exists(env_file):
            print(
                f"Configuration file '{env_file}' already exists. Skipping initialization."
            )
            return
...
""")
            print(
                f"Initialized default template configuration in: {os.path.abspath(env_file)}"
            )
        ...
        print(
            f"Loaded config file: {os.path.abspath(env_file) if os.path.exists(env_file) else 'None (using environment/defaults)'}"
        )
        ...
        if not args.key or not args.value:
            print(
                "Error: config set requires key and value. E.g. 'codivus config set default_model gpt-4o'",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            "Error: Unknown config subcommand. Use: init, show, or set.",
            file=sys.stderr,
        )
        sys.exit(1)
