...
from codereview.models import ReviewResult, RepositoryReviewResult, Summary, Score, RepositorySummary
...
def test_repo_subcommand(mock_isdir, mock_exists, mock_reviewer_class, dummy_repo_result):
    mock_exists.return_value = True
...
def test_security_subcommand(mock_exists, mock_reviewer_class, dummy_result):
    ...
    with patch.object(sys, "argv", test_args):
        main()
        mock_reviewer.review_file.assert_called_once_with(os.path.abspath("a.py"), category_focus="security")
...
def test_report_subcommand(mock_exists, mock_output_result, dummy_result, tmp_path):
    ...
    test_args = ["codivus", "report", str(report_file), "--format", "html", "--output", "out.html"]
    with patch.object(sys, "argv", test_args):
        main()
