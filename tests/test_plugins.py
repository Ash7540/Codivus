...
from codereview.models import CodeContext, Issue, Suggestion, ReviewResult, FileStats
...
def test_manual_plugin_registration(tmp_path):
    ...
    def generate_review_spy(code_context, static_issues=None, modified_lines=None, category_focus=None, prompt_modifier=None):
        if prompt_modifier:
            # Trigger the modifier so we can check it
            prompt_modifier("test prompt")
        return original_generate_review(code_context, static_issues, modified_lines, category_focus, prompt_modifier)
